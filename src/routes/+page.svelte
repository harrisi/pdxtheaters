<script lang="ts">
  ...
  import ScreeningsList from './ScreeningsList.svelte';
...
<!--<ScreeningsList screenings={data.screenings} /> -->
<List>
  {#each screenings as screening, i}
    {#if groupBy == 'showtime'}
      {#if i == 0}
        <dt>{dayjs(screening.showtime).format('h:mma')}</dt>
      {:else if (i > 0 && screening.showtime !== screenings[i-1].showtime)}
        <dt>{dayjs(screening.showtime).format('h:mma')}</dt>
      {/if}
    {:else if groupBy == 'theater'}
      {#if i == 0}
        <dt>{screening.theater_name}</dt>
      {:else if (i > 0 && screening.theater_name !== screenings[i-1].theater_name)}
        <dt>{screening.theater_name}</dt>
      {/if}
    {:else if groupBy == 'movie'}
      {#if i == 0}
        <dt>{screening.movie_title}</dt>
      {:else if (i > 0 && screening.movie_title !== screenings[i-1].movie_title)}
        <dt>{screening.movie_title}</dt>
      {/if}
    {/if}
